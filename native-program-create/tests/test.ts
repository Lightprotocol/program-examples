...
const PROGRAM_ID = new web3.PublicKey("rent4o4eAiMbxpkAM1HeXzks9YeGuz18SEgXEizVvPq");
...
describe("native-program-create", () => {
  it("create compressed account", async () => {
    ...
    const accountData = borsh.deserialize(
      MyCompressedAccount.schema,
      MyCompressedAccount,
      Buffer.from(compressedAccount.data.data),
    );
    console.log("compressed account: ", compressedAccount);
    console.log("decoded account owner: ", new web3.PublicKey(accountData.owner).toBase58());
...
async function createCompressedAccount(
  ...
  const instructionDataSchema = new Map<any, any>([
    [
      Object,
      {
        kind: "struct",
