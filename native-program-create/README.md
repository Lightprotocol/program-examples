# Create Compressed Account Program (Native Rust)

A Native Solana program to create a zk-compressed account with a custom message field.
...
The compiled `.so` file will be in `target/deploy/native_program_create.so`
...
2. Start the test validator
   ...
   ```bash
   light test-validator --sbf-program "rent4o4eAiMbxpkAM1HeXzks9YeGuz18SEgXEizVvPq" ./target/deploy/native_program_create.so
   ```
