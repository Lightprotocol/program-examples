# Reinitialize Compressed Account Program (Anchor)

A program built with the Anchor framework to reinitialize a previously closed zk-compressed account at the same address with default values.
...
2. Start the test validator
   ...
   ```bash
   light test-validator --sbf-program "2Txp954YU4HmX8ZpqHVq7PCLf8pMkHfYoNWxtB9mL5gW" ./target/deploy/anchor_program_reinit.so
   ```
