# Burn Compressed Account Program (Anchor)

A program built with the Anchor framework to permanently destroy a zk-compressed account. Unlike close, burned accounts cannot be reinitialized.
...
2. Start the test validator
   ...
   ```bash
   light test-validator --sbf-program "3VjoDzKqub6USAHbj2chMZb8396SdmmhcdosaoqPN9Rr" ./target/deploy/anchor_program_burn.so
   ```
