# Update Compressed Account Program (Native Rust)

A Native Solana program to update a zk-compressed account's message field.
...
The compiled `.so` file will be in `target/deploy/native_program_update.so`
...
2. Start the test validator
   ...
   ```bash
   light test-validator --sbf-program "rent4o4eAiMbxpkAM1HeXzks9YeGuz18SEgXEizVvPq" ./target/deploy/native_program_update.so
   ```
