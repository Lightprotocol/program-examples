...
const UPDATE_PROGRAM_ID = new web3.PublicKey("rent4o4eAiMbxpkAM1HeXzks9YeGuz18SEgXEizVvPq");
...
describe("native-program-update", () => {
  it("create and update compressed account", async () => {
    ...
    // Create compressed account with initial message
    ...
    let accountData = borsh.deserialize(
      MyCompressedAccount.schema,
      MyCompressedAccount,
      Buffer.from(compressedAccount.data.data),
    );
    console.log("Initial message: ", accountData.message);
    ...
    accountData = borsh.deserialize(
      MyCompressedAccount.schema,
      MyCompressedAccount,
      Buffer.from(compressedAccount.data.data),
    );
    console.log("Updated message: ", accountData.message);
  });
});
...
async function updateCompressedAccount(
  ...
  const systemAccountConfig = SystemAccountMetaConfig.new(UPDATE_PROGRAM_ID);
  let remainingAccounts =
    PackedAccounts.newWithSystemAccounts(systemAccountConfig);
  ...
  const serializedData = borsh.serialize(instructionDataSchema, instructionData);
  const instructionDiscriminator = Buffer.from([1]); // InstructionType::Update = 1
