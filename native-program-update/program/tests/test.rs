...
use native_program_update::{MyCompressedAccount, UpdateInstructionData};
use solana_sdk::{
...
async fn test_update() {
    let config = ProgramTestConfig::new(true, Some(vec![
        ...
        ("native_program_update", native_program_update::ID),
    ]));
...
pub async fn update_compressed_account(
    ...
    let system_account_meta_config = SystemAccountMetaConfig::new(native_program_update::ID);
    ...
    let instruction = Instruction {
        program_id: native_program_update::ID,
        accounts: account_metas,
        data: [
            &[native_program_update::InstructionType::Update as u8][..],
            &inputs[..],
        ]
        .concat(),
    };
