...
const REINIT_PROGRAM_ID = new web3.PublicKey("rent4o4eAiMbxpkAM1HeXzks9YeGuz18SEgXEizVvPq");

describe("native-program-reinit", () => {
  it("create, close, and reinit compressed account", async () => {
    ...
    compressedAccount = await rpc.getCompressedAccount(bn(address.toBytes()));
    console.log("Account reinitialized, data should be default:", compressedAccount.data);
  });
});
...
async function closeCompressedAccount(rpc: Rpc, compressedAccount: any, outputMerkleTree: web3.PublicKey, signer: web3.Keypair, currentMessage: string) {
  ...
  const accountMeta = { treeInfo: { rootIndex: proofRpcResult.rootIndices[0], treeIndex: stateMerkleTreeIndex }, address: Array.from(compressedAccount.address), outputStateTreeIndex: outputMerkleTreeIndex };
  const instructionData = { proof: proofRpcResult.compressedProof, accountMeta, currentMessage };

  const instructionDataSchema = new Map([
    [Object, { kind: "struct", fields: [["proof", { kind: "option", type: "CompressedProof" }], ["accountMeta", "CompressedAccountMeta"], ["currentMessage", "string"]] }],
    ["CompressedProof", { kind: "struct", fields: [["a", [32]], ["b", [64]], ["c", [32]]] }],
    ["CompressedAccountMeta", { kind: "struct", fields: [["treeInfo", "PackedTreeInfo"], ["address", [32]], ["outputStateTreeIndex", "u8"]] }],
...
async function reinitCompressedAccount(rpc: Rpc, compressedAccount: any, outputMerkleTree: web3.PublicKey, signer: web3.Keypair) {
  const proofRpcResult = await rpc.getValidityProofV0([bn(compressedAccount.hash)], []);
  const systemAccountConfig = SystemAccountMetaConfig.new(REINIT_PROGRAM_ID);
  let remainingAccounts = PackedAccounts.newWithSystemAccounts(systemAccountConfig);
  ...
  const accountMeta = { treeInfo: { rootIndex: proofRpcResult.rootIndices[0], treeIndex: stateMerkleTreeIndex }, address: Array.from(compressedAccount.address), outputStateTreeIndex: outputMerkleTreeIndex };
  const instructionData = { proof: proofRpcResult.compressedProof, accountMeta };

  const instructionDataSchema = new Map([
    [Object, { kind: "struct", fields: [["proof", { kind: "option", type: "CompressedProof" }], ["accountMeta", "CompressedAccountMeta"]] }],
    ["CompressedProof", { kind: "struct", fields: [["a", [32]], ["b", [64]], ["c", [32]]] }],
    ["CompressedAccountMeta", { kind: "struct", fields: [["treeInfo", "PackedTreeInfo"], ["address", [32]], ["outputStateTreeIndex", "u8"]] }],
    ["PackedTreeInfo", { kind: "struct", fields: [["rootIndex", "u16"], ["treeIndex", "u8"]] }],
  ]);
