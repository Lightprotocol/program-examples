# Reinitialize Compressed Account Program (Native Rust)

A Native Solana program to reinitialize a previously closed zk-compressed account at the same address with default values.
...
The compiled `.so` file will be in `target/deploy/native_program_reinit.so`
...
2. Start the test validator
   ...
   ```bash
   light test-validator --sbf-program "rent4o4eAiMbxpkAM1HeXzks9YeGuz18SEgXEizVvPq" ./target/deploy/native_program_reinit.so
   ```
