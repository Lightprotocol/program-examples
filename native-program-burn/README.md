# Burn Compressed Account Program (Native Rust)

A Native Solana program to permanently destroy a zk-compressed account. Unlike close, burned accounts cannot be reinitialized.
...
The compiled `.so` file will be in `target/deploy/native_program_burn.so`
...
2. Start the test validator
   ...
   ```bash
   light test-validator --sbf-program "rent4o4eAiMbxpkAM1HeXzks9YeGuz18SEgXEizVvPq" ./target/deploy/native_program_burn.so
   ```
